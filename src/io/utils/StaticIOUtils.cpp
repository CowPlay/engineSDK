...
namespace irrgame
{
	namespace io
	{
		...
		s32 StaticIOUtils::isFileExtension(const core::stringc& filename,
		...
		//! search if a filename has a proper extension
		bool StaticIOUtils::hasFileExtension(const core::stringc& filename,
				const core::stringc& ext0, const core::stringc& ext1,
				const core::stringc& ext2)
		{
		...
		core::stringc& StaticIOUtils::cutFilenameExtension(core::stringc &dest,
		...
		s32 StaticIOUtils::isspace(s32 c)
		{
			return c == ' ' || c == '\f' || c == '\n' || c == '\r' || c == '\t'
					|| c == '\v';
		}
		s32 StaticIOUtils::isupper(s32 c)
		{
			return c >= 'A' && c <= 'Z';
		}
		...
		s32 StaticIOUtils::getByteFromHex(c8 h)
		{
			if (h >= '0' && h <= '9')
				return h - '0';
		...
		void StaticIOUtils::getHexStrFromByte(c8 byte, c8* out)
		{
			s32 b = (byte & 0xf0) >> 4;
