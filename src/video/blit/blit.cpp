	...
	namespace video
	{
		...
		void executeBlit_TextureCopy_32_to_16(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					...
					const u32 s = PixelLerp32(src[dx] | 0xFF000000,
							extractAlpha(src[dx]));
					dst[dx] =
		...
		void executeBlit_TextureCopy_24_to_16(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				...
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					dst[dx] = SharedColorConverter::getInstance().RGBA16(s[0],
							s[1], s[2]);
					s += 3;
				}
		...
		void executeBlit_TextureCopy_16_to_32(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				...
				{
					dst[dx] =
							SharedColorConverter::getInstance().A1R5G5B5toA8R8G8B8(
									src[dx]);
				}
		...
		void executeBlit_TextureBlendColor_16_to_16(const SBlitJob * job)
		{
			...
			u16 blend = SharedColorConverter::getInstance().A8R8G8B8toA1R5G5B5(
					job->argb);
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
		...
		void executeBlit_Color_16_to_16(const SBlitJob * job)
		{
			...
			u16 c0 = SharedColorConverter::getInstance().A8R8G8B8toA1R5G5B5(
					job->argb);
			u32 c = c0 | c0 << 16;
			...
			while (b->operation != BLITTER_INVALID)
			{
				if (b->operation == operation)
				{
					if ((b->destFormat == -1 || b->destFormat == destFormat)
							&& (b->sourceFormat == -1
									|| b->sourceFormat == sourceFormat))
						return b->func;
					else if (b->destFormat == -2
				...
				const recti *sourceClipping, u32 argb)
		{
			...
			if (source)
			{
				...
				job.src = (void*) ((u8*) source->lock()
						+ (job.Source.y0 * job.srcPitch)
						+ (job.Source.x0 * job.srcPixelMul));
			}
			else
			{
			...
			job.dst = (void*) ((u8*) dest->lock() + (job.Dest.y0 * job.dstPitch)
					+ (job.Dest.x0 * job.dstPixelMul));
