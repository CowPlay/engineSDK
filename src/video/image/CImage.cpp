...
namespace irrgame
{
	namespace video
	{
		...
		void CImage::setPixel(u32 x, u32 y, const SColor &color, bool blend)
		{
			...
			switch (Format)
			{
				...
				case ECF_R5G6B5:
				{
					u16 * dest = (u16*) (Data + (y * Pitch) + (x << 1));
					*dest =
							SharedColorConverter::getInstance().A8R8G8B8toR5G6B5(
									color.color);
				}
					break;
		...
		SColor CImage::getPixel(u32 x, u32 y) const
		{
			...
			{
				case ECF_A1R5G5B5:
					return SharedColorConverter::getInstance().A1R5G5B5toA8R8G8B8(
							((u16*) Data)[y * Size.Width + x]);
				case ECF_R5G6B5:
					return SharedColorConverter::getInstance().R5G6B5toA8R8G8B8(
							((u16*) Data)[y * Size.Width + x]);
				case ECF_A8R8G8B8:
				...
				const recti* clipRect)
		{
			...
			Blit(
					color.color == 0xFFFFFFFF ?
							...
							BLITTER_TEXTURE_ALPHA_COLOR_BLEND, target, clipRect,
					&pos, this, &sourceRect, color.color);
		}
				...
				EColorFormat format, u32 pitch)
		{
			...
			if (Format == format && Size.Width == width
					&& Size.Height == height)
			{
				if (pitch == Pitch)
				{
			...
			for (u32 y = 0; y < height; ++y)
			{
				...
				{
					SharedColorConverter::getInstance().convert_viaFormat(
							Data + syval + ((s32) sx) * BytesPerPixel, Format,
							1, ((u8*) target) + yval + (x * bpp), format);
					sx += sourceXStep;
				}
				sy += sourceYStep;
				syval = ((s32) sy) * Pitch;
		...
		void CImage::copyToScaling(IImage* target)
		{
			...
			copyToScaling(target->lock(), targetSize.Width, targetSize.Height,
