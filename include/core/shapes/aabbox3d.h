	...
	namespace core
	{
		...
		class aabbox3d
		{
			public:
				...
				static const aabbox3d& getEmpty();
			...
			public:
				...
				EIntersectionRelation3D classifyPlaneRelation(
		...
		//! Returns empty bounding box
		template<class T>
		inline const aabbox3d<T>& aabbox3d<T>::getEmpty(void)
		{
			static const aabbox3d empty;
			return empty;
		}

		//! Default Constructor.
		template<class T>
		inline aabbox3d<T>::aabbox3d() :
		...
		//! Constructor with min edge and max edge.
		template<class T>
		inline aabbox3d<T>::aabbox3d(const vector3d<T>& min,
		...
		//! Constructor with only one point.
		template<class T>
		inline aabbox3d<T>::aabbox3d(const vector3d<T>& init) :
		...
		//! Constructor with min edge and max edge as single values, not vectors.
		template<class T>
		inline aabbox3d<T>::aabbox3d(T minx, T miny, T minz, T maxx, T maxy,
				T maxz) :
		...
		/** This means that there is no space between the min and max edge.
		 \return True if box is empty, else false. */
		template<class T>
		inline bool aabbox3d<T>::isEmpty() const
		{
		...
		//! Get the volume enclosed by the box in cubed units
		template<class T>
		inline T aabbox3d<T>::getVolume() const
		{
		...
		//! Get the surface area of the box in squared units
		template<class T>
		inline T aabbox3d<T>::getArea() const
		{
			const vector3d<T> e = getExtent();
		...
		/** \param edges: Pointer to array of 8 edges. */
		template<class T>
		inline void aabbox3d<T>::getEdges(vector3d<T> *edges) const
		{
			...
			edges[0].set(middle.X + diag.X, middle.Y + diag.Y,
					middle.Z + diag.Z);
			edges[1].set(middle.X + diag.X, middle.Y - diag.Y,
					middle.Z + diag.Z);
			edges[2].set(middle.X + diag.X, middle.Y + diag.Y,
					middle.Z - diag.Z);
			edges[3].set(middle.X + diag.X, middle.Y - diag.Y,
					middle.Z - diag.Z);
			edges[4].set(middle.X - diag.X, middle.Y + diag.Y,
					middle.Z + diag.Z);
			edges[5].set(middle.X - diag.X, middle.Y - diag.Y,
					middle.Z + diag.Z);
			edges[6].set(middle.X - diag.X, middle.Y + diag.Y,
					middle.Z - diag.Z);
			edges[7].set(middle.X - diag.X, middle.Y - diag.Y,
					middle.Z - diag.Z);
		}

		//! Repairs the box.
		template<class T>
		inline void aabbox3d<T>::repair()
		{
			T t;
		...
		//! Calculates a new interpolated bounding box.
		template<class T>
		inline aabbox3d<T> aabbox3d<T>::getInterpolated(
				const aabbox3d<T>& other, f32 d) const
		{
			f32 inv = 1.0f - d;
		...
		//! Determines if a point is within this box.
		template<class T>
		inline bool aabbox3d<T>::isPointInside(const vector3d<T>& p) const
		{
		...
		//! Determines if a point is within this box and not its borders.
		template<class T>
		inline bool aabbox3d<T>::isPointTotalInside(const vector3d<T>& p) const
		{
			return (p.X > MinEdge.X && p.X < MaxEdge.X && p.Y > MinEdge.Y
					&& p.Y < MaxEdge.Y && p.Z > MinEdge.Z && p.Z < MaxEdge.Z);
		}
		...
		//! Check if this box is completely inside the 'other' box.
		template<class T>
		inline bool aabbox3d<T>::isFullInside(const aabbox3d<T>& other) const
		{
		...
		//! Determines if the box intersects with another box.
		template<class T>
		inline bool aabbox3d<T>::intersectsWithBox(
				const aabbox3d<T>& other) const
		{
		...
		//! Tests if the box intersects with a line
		template<class T>
		inline bool aabbox3d<T>::intersectsWithLine(const line3d<T>& line) const
		{
			return intersectsWithLine(line.getMiddle(),
					line.getVector().normalize(), (T) (line.getLength() * 0.5));
		}

		//! Tests if the box intersects with a line
		template<class T>
		inline bool aabbox3d<T>::intersectsWithLine(
				const vector3d<T>& linemiddle, const vector3d<T>& linevect,
				T halflength) const
		{
		...
		/** \param plane Plane to classify relation to.
		 ...
		 ISREL3D_CLIPPED if it is on both sides of the plane. */
		template<class T>
		inline EIntersectionRelation3D aabbox3d<T>::classifyPlaneRelation(
				const plane3d<T>& plane) const
		{
