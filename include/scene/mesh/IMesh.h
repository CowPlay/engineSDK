...
#include "scene/mesh/EHardwareMapping.h"
#include "scene/mesh/EBufferType.h"
//#include "SMaterial.h"
...
namespace irrgame
{
	...
	namespace scene
	{
		...
		/** An IMesh is nothing more than a collection of some mesh buffers
		 (IMeshBuffer). A mesh is usually added to an IMeshSceneNode in order to be rendered.
		 */
		class IMesh: public IReferenceCounted
		{
			public:
				...
				//! adds a MeshBuffer
				virtual void addMeshBuffer(IMeshBuffer* buf) = 0;
				...
				virtual void setDirty(
						EBufferType buffer = EBT_VERTEX_AND_INDEX) = 0;
		};

		//! Mesh creator. Internal function. Please do not use. For create mesh use ISceneManager::getMesh();
